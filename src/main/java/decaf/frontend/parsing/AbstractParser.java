...
import decaf.frontend.tree.Tree.TArray;
...
abstract class AbstractParser {
    ...
    protected SemValue buildFuncType(SemValue first, List<SemValue> rest) {
        if (rest.isEmpty()) {
            return first;
        }
        ...
        for (var sv: rest)  {
            if (sv.intVal == 1)
                type = new TArray(type, first.pos);
            else
                type = new Tree.TLambda(type, sv.typeList);
        }
        return svType(type);
    }
